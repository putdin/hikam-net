...
class Siswa extends CI_Controller
{
	...
	public function update_kel($id_kelas)
	{
		...
		if (!$valid->run()) {
			$data = array(
				'title'     => 'Update IP : ' . $kelas->nama_kelas,
				'kelas'     => $kelas
			);
			$this->load->view('admin/template/header', $data);
			$this->load->view('admin/template/sidebar', $data);
			...
			$this->session->set_flashdata('message', 'Data IP dan ODP <b>' . $kelas->nama_kelas . '</b> telah diupdate');
			redirect('admin/siswa/kelas');
		}
	}
	...
	public function delete_kel($id_kelas)
	{
		...
		$this->session->set_flashdata('message', 'Data IP <b>' . $kelas->nama_kelas . '</b> telah dihapus');
		redirect('admin/siswa/kelas');
	}

	public function wa($nisn)
		{
			$siswa = $this->siswa_model->detail($nisn);
